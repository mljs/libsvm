...
module.exports = function (libsvm) {
  ...
  const svm_get_parameters = libsvm.cwrap('get_parameters', 'number', ['number']);
  ...
  class SVM {
     ...
     * Get the model's parameters as a js object
     * @return {object} The parameter object
     */
    getModelParameters() {
      if (!this.model) {
        throw new Error('Cannot get model parameters. No model was trained');
      }
      const svmParams = svm_get_parameters(this.model);
      console.log(
        new Uint8Array(libsvm.HEAP8.buffer.slice(svmParams, svmParams + 40))
      );
      const view = new DataView(libsvm.HEAP8.buffer, svmParams);
      const params = {};
      let byteCount = 0;
      params.type = String(view.getInt32(byteCount, true));
      byteCount += 4;
      params.kernel = String(view.getInt32(byteCount, true));
      byteCount += 4;
      params.degree = view.getInt32(byteCount, true);
      byteCount += 4;
      for (let i = 0; i < 8; i++) {
        console.log(view.getUint8(byteCount + i));
      }
      params.gamma = view.getFloat64(byteCount, true);
      byteCount += 8;
      params.coef0 = view.getFloat64(byteCount, true);
      byteCount += 8;
      params.cacheSize = view.getFloat64(byteCount, true);
      byteCount += 8;
      params.eps = view.getFloat64(byteCount, true);
      byteCount += 8;
      params.C = view.getFloat64(byteCount, true);
      byteCount += 8;
      params.nrWeight = view.getInt32(byteCount, true);
      byteCount += 4;
      // int* weight_label: 8 bytes
      byteCount += 8;
      // double* weight 8 bytes
      byteCount += 8;
      params.nu = view.getFloat64(byteCount, true);
      byteCount += 8;
      params.p = view.getFloat64(byteCount, true);
      byteCount += 8;
      params.shrinking = view.getInt32(byteCount, true);
      byteCount += 4;
      params.probability = view.getInt32(byteCount, true);
      byteCount += 4;
      return params;
